...
function llamarPaginaEditar(id){
	window.open("ingresarFicha.php?iddatos="+id, '_parent');
	}
        ...
        <table id="grid" class="display" cellspacing="0" width="99%">
            <thead>
               ...
               <tbody>
			    <?php
                        include("../config/conexion.php");
						$query_s= pg_query($conexion, "SELECT s.*
                        FROM solicitud s
                        WHERE NOT EXISTS (SELECT * FROM ficha_tecnica a 
                      WHERE s.idsolicitud = a.idsolicitud ) and s.idcliente='$idcliente'
            ...
            <tr>
                ...
                <td class="text-center"><a class='btn btn-info btn-lg' onClick="llamarPaginaEditar('<?php echo $fila[0]; ?>')"><span class="glyphicon glyphicon-edit"></span> Ingresar Ficha</a></td>
